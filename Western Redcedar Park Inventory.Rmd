...
We still have NA left over from observations that did not have any understory species observed, for our NMDS they need to be assigned with 0s for absence.
```{r}
...
```{r}
CombinedMatrix <- CombinedMatrix %>% filter(field.tree.health.condition != "no selection")
```
...
Check if dataset is transposed.
```{r}
dim(UnderstorySpeciesValuesMatrix)
```
